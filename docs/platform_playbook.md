...
- **ⓘ ZÁCHRANA grantů z obalů** — Haiku classify (`workflows/classify_wf.js`) správně vytáhne grant schovaný v „úřední desce / news / administrativě" (vyhlášení veřejné soutěže, „Vyhlášena výzva č. X"). Nespoléhej na regex/klíčová slova pro určení typu — `výběrové řízení`/`veřejná soutěž` jsou reálné granty. Klasifikuj OBSAHEM (LLM), ne obalem.
- **ⓘ ořez vstupu ZABÍJÍ vrstvu 2** — `amount` 27 % → 90 % po odstranění stropů (měřeno na 30 grantech). Vrstva 2 = `workflows/extract_wf.js`: plný text + plný text VŠECH příloh, 1 oportunita = 1 agent, žádné limity počtu/délky.
