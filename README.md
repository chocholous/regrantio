...
- **Živá aplikace (rozcestník větví + recept):** → **[chocholous.github.io/regrantio](https://chocholous.github.io/regrantio/)**
  Homepage = diagram logiky receptu + návod „jak přidat další data (Claude Code + Opus)" + seznam větví s merge-statusem. Každá větev má vlastní verzi appky — nejnovější je **`coverage-expansion-next`** (~2280 oportunit / 122 poskytovatelů k 2026-06-29: 14 krajů + ~30 měst + ministerstva + státní fondy + agentury GAČR/TAČR/NSA + nadace + LLM/deterministická vrstva 2 + Opus kategorie). Filtry: oblast · sektor/typ žadatele · cílová skupina · poskytovatel · kraj · forma · zdroj · spoluúčast · míra · typ dokumentu · status · výše. Detail nese doslovné citace (grounding) + odkazy na originály. Pages se nasazují per-branch přes GitHub Actions (`.github/workflows/pages.yml`).
...
```
- **core** (3 MB) — `opportunities.jsonl` (snapshot z doby zabalení; živý dataset je `data/opportunities_v2.jsonl`, ~2280 oportunit), harvest jsonl, configy, app
