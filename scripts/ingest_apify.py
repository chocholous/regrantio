...
# kraj HLAVNÍ weby (katalogy/výzvy) — app-portály dotace.{kraj}.cz ZÁMĚRNĚ NEjsou (jen žádost)
...
def is_opportunity(f, title):
    """True = konkrétní oportunita (výzva/program/mise); False = katalog/landing/news. (pozor: ne katalogy!)"""
    t = (title or "").strip()
    if not t or CATALOG.search(t):
        return False
    if NAMED.search(t):                       # pojmenovaný program/výzva → oportunita
        return True
    elig_ok = f.get("eligible_applicants") and len(str(f["eligible_applicants"])) > 40
    theme = f.get("focus_area") or f.get("oblast")
    sig = (f.get("deadline") or f.get("open_from") or f.get("cislo_vyzvy")
           or (f.get("deadliny") and len(f["deadliny"]) > 0) or (elig_ok and theme))
    mission_sig = f.get("mission") and f.get("jak_oslovit")
    return bool(sig or mission_sig)
