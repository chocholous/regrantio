...
6. **VŠECHNY limity/capy/ořezy v `limits.json` (root), NIKDY natvrdo v kódu/promptu.** Každý záznam: `value` (null=žádný), `unit`, `kind`, `why`, `used_in[]`, `wired`. Kód čte přes `scripts/limits.py` → `L('cesta.klic')`. **Coverage-capy (kolik zdrojů/stránek/příloh) MUSÍ logovat uříznutí** (`⚠ … zvyš --max-pages`), nikdy tiše — tiché capy = ztracená data (viz `_legacy_to_migrate`: `mv_cms body[:8000]`). Než zavedeš JAKÝKOLI limit, dej ho do registru + zdůvodni.
